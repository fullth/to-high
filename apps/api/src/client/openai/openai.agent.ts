...
${isAdviceRequested ? '⚠️ 사용자가 조언을 요청했습니다. 질문하지 말고 구체적인 조언만 제공하세요.' : '사용자의 마지막 선택/입력에 공감하면서, 질문이 필요한지 아니면 경청이 필요한지 판단하여 응답해주세요.'}`;
  ...
  async *generateOptionsStream(
    ...
    // 전체 결과를 한 번에 생성하고 question만 스트리밍처럼 전송
    const result = await this.generateOptions(context, currentStep, category, counselorType);

    // question을 한 글자씩 yield
    for (const char of result.question) {
      yield { type: 'question_chunk', content: char };
    }
    ...
    yield {
      ...
      canRequestFeedback: result.canRequestFeedback,
      responseType: result.responseType,
    };
  }
