...
export class OpenAIAgent {
  ...
  async generateOptions(
    ...
    // 조언 요청 감지
    const adviceRequestKeywords = ['조언', '정리해', '어떻게 하면', '어떻게 해야', '알려줘', '도와줘', '네 생각', '좋을까', '위로'];
    const isAdviceRequested = adviceRequestKeywords.some(keyword => lastUserInput.includes(keyword));

    // 디버깅 로그
    console.log('[generateOptions] userTurnCount:', userTurnCount);
...
${isAdviceRequested ? '⚠️ 사용자가 조언을 요청했습니다. 질문하지 말고 구체적인 조언만 제공하세요.' : '사용자의 마지막 선택/입력에 공감하면서, 질문이 필요한지 아니면 경청이 필요한지 판단하여 응답해주세요.'}`;
    ...
    console.log('[generateOptions] AI response:', {
  ...
  async *generateOptionsStream(
    ...
    const lastInput = context[context.length - 1];
    if (lastInput && this.isMeaninglessInput(lastInput)) {
      const question = counselorType?.startsWith('listening-') ? '네...'
        : counselorType?.startsWith('reaction-') ? '네네'
      ...
      const options = counselorType?.startsWith('listening-') ? ['더 말할게요', '여기까지만요']
        : counselorType?.startsWith('reaction-') ? ['그래서 말인데요', '다른 얘기할게요', '글쎄요...']
...
question만 생성하세요.`;
    ...
    for await (const chunk of stream) {
      ...
      if (content) {
        ...
        yield { type: 'question_chunk', content };
      }
    }
    ...
    if (shouldGiveAdvice && question.trim().endsWith('?')) {
      const sentences = question.split(/(?<=[.!?])\s*/);
      const nonQuestionSentences = sentences.filter((s: string) => !s.trim().endsWith('?'));
      question = nonQuestionSentences.length > 0 ? nonQuestionSentences.join(' ').trim() : '지금까지 말씀해주신 내용을 잘 들었어요. 제 생각을 말씀드릴게요.';
    }
...
위 질문에 답할 수 있는 8개의 선택지를 생성하세요.`;
    ...
    // "조언해줘" 강제 추가
    const adviceKeywords = ['조언', '정리', '해줘', '알려줘', '도와줘', '방법', '어떻게'];
    const hasAdviceOption = options.some((opt: string) => adviceKeywords.some(keyword => opt.includes(keyword)));
    if (contextCount >= 2 && !hasAdviceOption && options.length > 0) {
      const adviceOptions = ['조언해줘', '정리해줘', '어떻게 하면 좋을까', '네 생각을 말해줘'];
      options = [...options.slice(0, -1), adviceOptions[Math.floor(Math.random() * adviceOptions.length)]];
    }
