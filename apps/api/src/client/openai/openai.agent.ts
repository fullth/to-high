...
사용자의 마지막 선택/입력에 공감하면서 다음 질문과 선택지를 생성해주세요.`;
  ...
  async *generateOptionsStream(
    ...
    category?: Category,
    counselorType?: CounselorType,
  ): AsyncGenerator<{ type: string; content?: string; options?: string[]; canProceedToResponse?: boolean; canRequestFeedback?: boolean }> {
    const contextCount = context.length;
    const canRequestFeedback = contextCount >= PROMPT_CONFIG.MIN_CONTEXT_FOR_FEEDBACK;
    ...
    const lastInput = context[context.length - 1];
    if (lastInput && this.isMeaninglessInput(lastInput)) {
      const question = counselorType === 'listening' ? '네...'
        : counselorType === 'reaction' ? '네네'
      ...
      const options = counselorType === 'listening' ? ['더 말할게요', '여기까지만요']
        : counselorType === 'reaction' ? ['그래서 말인데요', '다른 얘기할게요', '글쎄요...']
    ...
    if (!this.hasApiKey) {
      const question = '말씀해주셔서 감사해요. 조금 더 이야기해주실 수 있을까요?';
      for (const char of question) {
        yield { type: 'question_chunk', content: char };
      }
      yield {
...
순수한 텍스트 질문만 작성하세요.`;
    ...
    if (allQuestionsRecently && question.endsWith('?')) {
      const sentences = question.split(/(?<=[.!?])\s*/);
      const nonQuestionSentences = sentences.filter((s: string) => !s.trim().endsWith('?'));
      question = nonQuestionSentences.length > 0 ? nonQuestionSentences.join(' ').trim() : '네, 잘 들었어요. 천천히 이야기해주세요.';
    }

    // options 별도 생성 (JSON 모드)
    const modeOptionsPrompt = counselorType ? COUNSELOR_MODE_OPTIONS_PROMPTS[counselorType] : `[질문 모드 선택지] 존댓말체(~요), 8개, 15자 이내
...
위 질문에 답할 수 있는 8개의 선택지를 **모두 부드러운 존댓말(~요체)로** 생성하세요.`;
    ...
    const optionsResponse = await this.openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: 'JSON 형식으로 응답: {"options": ["옵션1", "옵션2", ...]}. 모든 옵션은 반드시 존댓말 ~요체로.' },
      ],
      response_format: { type: 'json_object' },
      temperature: PROMPT_CONFIG.TEMPERATURE_OPTIONS,
    });

    const optionsResult = JSON.parse(optionsResponse.choices[0].message.content || '{"options": []}');
