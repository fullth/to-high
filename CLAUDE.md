...
| 2026-06-13 | 무료-우선 런칭: 세션 paywall 비활성화(`FREE_LAUNCH_MODE`) + 랜딩 nav '멤버십' 링크 제거 | app/chat/chat.service.ts, chat.service.spec.ts, components/landing/wirocare-landing.tsx, product-principles.md | 초기 사용자 유입 우선(V3 > V7). 구독 한도(무료 3권+티어)는 플래그 false로 즉시 복원 가능. 구독 시스템은 subscribe 페이지에 '준비중'으로 유지(코드 보존). 사용자 승인 받음 |
